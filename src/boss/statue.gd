...
func add_knockback(force: Vector2):
	force += force.length() * Vector2.UP * 0.3
	if physics_mode:
		rigid_body.apply_central_impulse(force * force_multiplier() ) 
	else:
...
func _process(_delta: float) -> void:
	rigid_body.freeze = Hitstun.paused
...
func force_multiplier():
	return log(total_damage_recieved)
...
func _on_bounce_detector_bounced() -> void:
	if rigid_body.global_position.y < 200:
